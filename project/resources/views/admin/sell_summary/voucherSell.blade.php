...
@section('title', trans('Voucher Payment'))
@section('content')

    <div class="page-header card card-primary m-0 m-md-4 my-4 m-md-0 p-5 shadow">
        <div class="row justify-content-between">
            <div class="col-md-12">
                <form action="{{route('admin.voucherSell.search')}}" method="get">
                    <div class="row">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-2">
                            <select class="form-control mb-3" name="voucher_status"
                                    aria-label=".form-select-lg example">
                                <option value="0" @if (@request()->voucher_status==0) selected @endif>@lang('All Voucher')
                                </option>
                                <option value="1" @if (@request()->voucher_status==1) selected @endif>@lang('Complete Voucher')
                                </option>
                                <option value="2" @if (@request()->voucher_status==2) selected @endif>
                                    @lang('Pending Voucher')</option>
                            </select>
    ...
    <div class="card card-primary m-0 m-md-4 my-4 m-md-0 shadow">
        <div class="card-body">
            <table class="categories-show-table table table-hover table-striped table-bordered">
                ...
                @forelse($voucherSell as $k => $row)
                    <tr>
                        ...
                        <td data-label="@lang('Voucher')">@lang(@optional($row->voucher)->details->name)</td>
                        ...
                        <td data-label="@lang('More')">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary btn-icon edit_button"
                                    ...
                                    data-route="{{route('admin.voucherSell.action',$row->id)}}"
                                    data-info="{{$details}}"
                                    data-id="{{$row->id}}"
            ...
            {{ $voucherSell->links('partials.pagination') }}
        </div>
    </div>
