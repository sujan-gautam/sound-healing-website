            ...
            <div class="row justify-content-between bg-gradient">
                <div class="col-md-12">
                    <div class="contact-box mb-3 mx-2">
                        <form action="{{route('user.giftCardOrder.search')}}" method="get">
                            <div class="row">
                                <div class="col-md-4">
            ...
            <div class="col-md-12">
                <div class="table-responsive">
                    <table class="table table-bordered transection__table mt-5" id="service-table">
                        ...
                        <tbody>
                        @forelse($giftCardOrders as $k => $row)
                            <tr>
                                ...
                                <td data-label="@lang('Price')" ><span class="font-weight-bold">{{config('basic.currency_symbol')}}</span>{{getAmount($row->price)}}
                                    @if(0 <$row->discount)
                                        <sup class="badge-light badge-pill ">  {{config('basic.currency_symbol')}}{{getAmount($row->discount)}} @lang('Off')</sup>
                                    @endif
                                </td>
                                ...
                                <td data-label="@lang('More')">
                                    ...
                                    <button type="button" class="btn btn-custom btn-icon edit_button"
                                            data-bs-toggle="modal" data-bs-target="#codeShow"
                                            data-info="{{$details}}"
                                            data-id="{{$row->id}}"
                                    >
                                        <i class="fa fa-eye"></i>
                ...
                {{ $giftCardOrders->appends($_GET)->links($theme.'partials.pagination') }}
            </div>
        </div>
