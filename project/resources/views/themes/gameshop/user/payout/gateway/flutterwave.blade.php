...
@push('script')
    <script type="text/javascript">
        ...
        $(document).on("change", ".transfer-currency", function () {
            ...
            var output = null;
            $('#showInfo').html('');
            output = `<li class="preview-list-group-item font-weight-bold bg-transparent">@lang('Exchange rate') :
                        <span
                            class="float-right text-danger">1 ${baseCurrency} = ${rate} ${currencyCode}</span>
                    </li>
                    <li class="preview-list-group-item font-weight-bold bg-transparent">@lang('You will get:') :
                        <span
                            class="float-right text-danger">${getAmount.toFixed(2)} ${currencyCode}</span>
                    </li>`
