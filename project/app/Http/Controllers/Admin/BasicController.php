...
class BasicController extends Controller
{
    ...
    public function updateConfigure(Request $request)
    {
        ...
        $request->validate([
            ...
            'payment_expired' => 'required|integer|min:2',
            'payment_released' => 'required|integer|min:0'
        ]);

        config(['basic.site_title' => $reqData['site_title']]);
        config(['basic.base_color' =>  $reqData['base_color']]);
        config(['basic.base_secondary' =>  $reqData['base_secondary']]);
        ...
        config(['basic.top_up' => (int) $reqData['top_up']]);
        config(['basic.voucher' => (int) $reqData['voucher']]);
        config(['basic.gift_card' => (int) $reqData['gift_card']]);
        config(['basic.sell_post' => (int) $reqData['sell_post']]);
        config(['basic.payment_expired' => (int) $reqData['payment_expired']]);
        config(['basic.payment_released' => (int) $reqData['payment_released']]);
    ...
    public function breadcrumbUpdate(Request $request)
    {
        ...
        if ($request->hasFile('form_image')) {
            try {
                $old = 'game-character5.png';
                $this->uploadImage($request->form_image, config('location.logo.path'), null, $old, null, $old);
            } catch (\Exception $exp) {
                return back()->with('error', 'Banner could not be uploaded.');
            }
        }
