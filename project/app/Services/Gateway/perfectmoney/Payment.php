...
class Payment
{
    public static function prepareData($order, $gateway)
    {
        ...
        $val['PAYMENT_AMOUNT'] = round($order->final_amount,2);
    ...
    public static function ipn($request, $gateway, $order = null, $trx = null, $type = null)
    {
        ...
        if ($hash == $hash2) {
            ...
            if ($request->PAYEE_ACCOUNT == trim($gateway->parameters->payee_account) && $unit == $order->gateway_currency && $amount == round($order->final_amount,2)&& $order->status == '0') {
